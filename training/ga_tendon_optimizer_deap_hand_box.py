...
class GAConfig:
    ...
    # Site parameters
    N_SITES_ARM_X = 2  # Number of connection sites per chopstick 
    N_SITES_ARM_Y = 2  # Number of connection sites per chopstick 
    ...
    N_SITES_OBJECT_X = 2  # Number of connection sites on object 
    N_SITES_OBJECT_Y = 2  # Number of connection sites on object 
    N_SITES_OBJECT_Z = 2  # Number of connection sites on object 
    ...
    BASE_CONNECTION_PROB = E_NUM_TENDON / (N_SITES_ARM * N_SITES_OBJECT) * 2  # Initial connection density
    ...
    LIFT_DELAY = 2         # seconds before applying the lift
...
def create_individual():
    ...
    # Adjacency (binary): 1 = connected, 0 = not connected
    for _ in range(n_cross_connections):
        genome.append(1 if random.random() < GAConfig.BASE_CONNECTION_PROB else 0)
    
    # Stiffness values
    for _ in range(n_cross_connections):
        genome.append(random.uniform(GAConfig.MIN_STIFFNESS, GAConfig.MAX_STIFFNESS))
    
    # Damping values
    for _ in range(n_cross_connections):
        genome.append(random.uniform(GAConfig.MIN_DAMPING, GAConfig.MAX_DAMPING))
...
def decode_genome(individual: creator.Individual) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    num_tendons = sum(individual[:n_cross_connections_arm_to_body + n_cross_connections_arm_to_body])
    ...
    # left arm
    for index in range(2*n_cross_connections_arm_to_body):
        if individual[ index ] == 1:
            # Get raw stiffness and damping from genome
            raw_stiffness = individual[n_cross_connections_arm_to_body + index]
            raw_damping = individual[n_cross_connections_arm_to_body * 2 + index]
...
def build_model_from_genome(G) -> MuJoCoSimulation:
    """
    ...
    random_orientation = quat_from_euler(0, 0, random.uniform(-math.pi/4, math.pi/4))
    ...
    builder.add_body("ghost_target", pos=[0.5, 0, 0.3],
                        quat=random_orientation,
                     geom_type="box", geom_size=[0.05, 0.05, 0.05],
                     free_joint=True, geom_rgba=[1,1,1,0.1], mass=100000, intersection=False)
...
def evaluate_fitness(individual: creator.Individual) -> Tuple[float,]:
    ...
    centre_dist = 1.0 / (1.0 + final_centre_dist)
...
def mutate_individual(individual: creator.Individual) -> Tuple[creator.Individual,]:
    ...
    """
    n_cross = GAConfig.N_SITES_ARM * GAConfig.N_SITES_OBJECT * 2
    ...
    # Mutate stiffness
    for i in range(n_cross, 2 * n_cross):
        individual[i] += random.gauss(GAConfig.GAUSSIAN_MU, GAConfig.GAUSSIAN_SIGMA)
        individual[i] = max(GAConfig.MIN_STIFFNESS, min(GAConfig.MAX_STIFFNESS, individual[i]))
    
    # Mutate damping
    for i in range(2 * n_cross, 3 * n_cross):
        individual[i] += random.gauss(GAConfig.GAUSSIAN_MU, GAConfig.GAUSSIAN_SIGMA / 10.0)
        individual[i] = max(GAConfig.MIN_DAMPING, min(GAConfig.MAX_DAMPING, individual[i]))
...
def crossover_individuals(ind1: creator.Individual, ind2: creator.Individual) -> Tuple[creator.Individual, creator.Individual]:
    ...
    """
    n_cross = GAConfig.N_SITES_OBJECT * GAConfig.N_SITES_ARM
    ...
    # Average crossover for stiffness and damping
    for i in range(n_cross, 3 * n_cross):
        if random.random() < 0.5:
