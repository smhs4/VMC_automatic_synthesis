...
to learn robust grasp configurations on a target block.

Fitness is based on:
- Holding grasp under disturbance
- Preventing target drop after support base is lowered
- Stability and robustness terms
...
class GAConfig:
    ...
    N_SITES_ARM_Z = 4  # Number of connection sites per chopstick
    ...
    # Fitness weights (grasp-and-hold while lowering disturbance)
    CONTACT_WEIGHT = 0
    HEIGHT_WEIGHT = 0
    HEIGHT_FINAL_RATIO = 0.0
    HEIGHT_INTEGRAL_RATIO = 0.0
    NO_DROP_WEIGHT = 8.0
    CENTRE_WEIGHT = 1
    ...
    TARGET_BASE_Y = 0.0
    TARGET_BASE_Z = 0.3
    TARGET_TRIAL_X_OFFSETS = (-0.05, 0.0, 0.05)
    TARGET_TRIAL_YAWS = (-np.pi/5, 0.0, np.pi/5)
    # Per-step disturbance during rollout (applied directly to target freejoint pose).
    # Keep disabled for base-lowering grasp tests to avoid artificial bouncing.
    TARGET_STEP_JITTER_ENABLED = False
    ...
    # Grasp-hold stress test setup.
    GRASP_FREEZE_TIME = 3.0        # seconds: latch grip and lock arm joints
    HOLD_POST_FREEZE_CTRL = False  # stale frozen torques can inject energy; disabled by default
    LOCK_JOINTS_AFTER_FREEZE = False  # hard qpos locking can destabilize; off by default
    POSE_HOLD_AFTER_FREEZE = True  # add PD hold to resist object-induced tracking
    POSE_HOLD_KP = 30.0
    POSE_HOLD_KD = 6.0
    BASE_LOWER_DISTANCE = 0.10     # meters: lower support base after freeze
    BASE_LOWER_DURATION = 3.0      # seconds: smoother ramp to lower base
    BASE_LOWER_START_DELAY = 0.2   # seconds: let grasp settle before lowering support
    # Contact/target damping to reduce bounce during the lowering test.
    CONTACT_SOLREF = [0.04, 2.0]   # [timeconst, dampratio] for softer, overdamped contacts
    CONTACT_SOLIMP = [0.95, 0.99, 0.001, 0.5, 2.0]
    TARGET_ANGULAR_DOF_DAMPING = 0.8
    TARGET_LINEAR_DOF_DAMPING = 2.5
    PLATFORM_INITIAL_Z = 0.1       # initial z for support base
    PLATFORM_SLIDE_JOINT = "platform_joint_001"
...
def create_individual():
    ...
    for i in range(len(genome)):
        ...
        length = np.random.exponential(scale=0.01)
...
class TendonTorqueController:
    ...
    def __init__(
        ...
        freeze_time: float = GAConfig.GRASP_FREEZE_TIME,
        hold_post_freeze_ctrl: bool = GAConfig.HOLD_POST_FREEZE_CTRL,
        ...
        self.hold_post_freeze_ctrl = hold_post_freeze_ctrl
        self.torque_clip = torque_clip
        self.freeze_armed = False
        self.frozen_ctrl: Optional[np.ndarray] = None
        self.frozen_joint_qpos: Optional[np.ndarray] = None
        self.platform_start_qpos: float = 0.0
        ...
        self.lock_joint_qadr: List[int] = []
        self.lock_joint_dofadr: List[int] = []
        for j_name in ROBOT_JOINT_NAMES:
            j_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, j_name)
            if j_id == -1:
                continue
            self.lock_joint_qadr.append(int(self.model.jnt_qposadr[j_id]))
            self.lock_joint_dofadr.append(int(self.model.jnt_dofadr[j_id]))

        p_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, GAConfig.PLATFORM_SLIDE_JOINT)
        if p_id != -1:
            self.platform_qadr = int(self.model.jnt_qposadr[p_id])
            self.platform_dofadr = int(self.model.jnt_dofadr[p_id])
        else:
            self.platform_qadr = -1
            self.platform_dofadr = -1

    def arm_freeze(self):
        """Latch current actuator controls for post-freeze replay."""
        if self.freeze_armed:
            return
        self.freeze_armed = True
        self.frozen_ctrl = self.data.ctrl.copy()
        self.frozen_joint_qpos = np.array(
            [self.data.qpos[qadr] for qadr in self.lock_joint_qadr],
            dtype=np.float64,
        )
        if self.platform_qadr != -1:
            self.platform_start_qpos = float(self.data.qpos[self.platform_qadr])

    def callback(self, _sim: MuJoCoSimulation):
        if self.freeze_armed or self.data.time >= self.freeze_time:
            if not self.freeze_armed:
                self.arm_freeze()
            if self.hold_post_freeze_ctrl and self.frozen_ctrl is not None:
                self.data.ctrl[:] = self.frozen_ctrl

            # Lower support base after freeze to stress-test grasp quality.
            if self.platform_qadr != -1:
                self.data.qpos[self.platform_qadr] = -1
        ...
        for idx, t_id in enumerate(self.tendon_ids):
            ...
            velocity = float(self.data.ten_velocity[t_id])
...
def build_model_from_genome(
    ...
    if target_pos is not None:
        ...
        target_position = [random.uniform(GAConfig.TARGET_BASE_X-0.05, GAConfig.TARGET_BASE_X+0.05), GAConfig.TARGET_BASE_Y, GAConfig.TARGET_BASE_Z]
    # Add a platform and target object
    t0 = time.perf_counter()
    builder.add_body("platform", pos=[0.5, 0, GAConfig.PLATFORM_INITIAL_Z],
                        geom_type="box", geom_size=[0.2, 0.2, 0.1],
                        free_joint=False, joints=[[0, 0, 1]],
                        geom_rgba=[0.7,0.5,0.5,1], mass=10, )
    ...
    # No shoulder/lifter assist tendons are added for grasp-hold stress tests.
    ...
    torque_controller = TendonTorqueController(
        ...
        freeze_time=GAConfig.GRASP_FREEZE_TIME,
        hold_post_freeze_ctrl=GAConfig.HOLD_POST_FREEZE_CTRL,
    ...
    sim._torque_controller = torque_controller
...
def evaluate_fitness(individual: creator.Individual) -> Tuple[float,]:
    ...
    trial_num = min(len(trial_angles), len(trial_x_positions), len(arm_perturbations))

    for angle, x_pos, arm_offsets in zip(
        trial_angles[:trial_num],
        trial_x_positions[:trial_num],
        arm_perturbations[:trial_num],
    ):
        sim = build_model_from_genome(
            G,
            angle=angle,
            target_pos=[x_pos, GAConfig.TARGET_BASE_Y, GAConfig.TARGET_BASE_Z],
        ...
        height_at_lower_start = None
        min_height_after_lower = None
        ...
        torque_controller = getattr(sim, "_torque_controller", None)
        ...
        for _ in range(num_steps):
            ...
            if (
                not freeze_latched and
                (
                    (torque_controller is not None and torque_controller.freeze_armed) or
                    (sim.data.time >= freeze_time)
                )
            ):
                freeze_latched = True
                height_at_lower_start = target.pos[2]
                min_height_after_lower = target.pos[2]

            # Apply small random pose disturbance to target during rollout for robustness.
            # Disable jitter after freeze so the base-lowering grasp test is isolated.
            if (not freeze_latched) and target_qadr != -1 and GAConfig.TARGET_STEP_JITTER_ENABLED:
            ...
            if freeze_latched and min_height_after_lower is not None:
                min_height_after_lower = min(min_height_after_lower, target.pos[2])
        

        # Final height (kept for optional diagnostics; not weighted by default here)
        ...
        # Main grasp objective: once lowering starts, box should not drop.
        if height_at_lower_start is None:
            height_at_lower_start = target.pos[2]
            min_height_after_lower = target.pos[2]
        drop_after_lower = max(0.0, height_at_lower_start - min_height_after_lower)
        no_drop_score = 1.0 / (1.0 + drop_after_lower)
        ...
        trial_score = (
            ...
            GAConfig.NO_DROP_WEIGHT * no_drop_score +
...
def run_evolution(visualize: bool = False, resume: str = None, plot_stats: bool = False, 
    ...
    log(f"  Target Step Jitter Enabled: {GAConfig.TARGET_STEP_JITTER_ENABLED}")
    ...
    log(f"  Grasp Freeze Time:    {GAConfig.GRASP_FREEZE_TIME} s")
    log(f"  Hold Frozen Ctrl:     {GAConfig.HOLD_POST_FREEZE_CTRL}")
    log(f"  Lock Joints After Freeze: {GAConfig.LOCK_JOINTS_AFTER_FREEZE}")
    log(f"  Pose Hold After Freeze: {GAConfig.POSE_HOLD_AFTER_FREEZE}")
    log(f"  Pose Hold Gains (Kp,Kd): ({GAConfig.POSE_HOLD_KP}, {GAConfig.POSE_HOLD_KD})")
    log(f"  Base Lower Distance:  {GAConfig.BASE_LOWER_DISTANCE} m")
    log(f"  Base Lower Duration:  {GAConfig.BASE_LOWER_DURATION} s")
    log(f"  Base Lower Start Delay: {GAConfig.BASE_LOWER_START_DELAY} s")
    log(f"  Contact Solref:       {GAConfig.CONTACT_SOLREF}")
    log(f"  Contact Solimp:       {GAConfig.CONTACT_SOLIMP}")
    log(f"  Target DOF Damping (ang/lin): {GAConfig.TARGET_ANGULAR_DOF_DAMPING}/{GAConfig.TARGET_LINEAR_DOF_DAMPING}")
    log("")
    log(f"  Contact Weight:       {GAConfig.CONTACT_WEIGHT}")
    log(f"  Height Weight:        {GAConfig.HEIGHT_WEIGHT}")
    log(f"  No-Drop Weight:       {GAConfig.NO_DROP_WEIGHT}")
    ...
    config_dict = {
        ...
        'target_step_jitter_enabled': GAConfig.TARGET_STEP_JITTER_ENABLED,
        ...
        'grasp_freeze_time': GAConfig.GRASP_FREEZE_TIME,
        'hold_post_freeze_ctrl': GAConfig.HOLD_POST_FREEZE_CTRL,
        'lock_joints_after_freeze': GAConfig.LOCK_JOINTS_AFTER_FREEZE,
        'pose_hold_after_freeze': GAConfig.POSE_HOLD_AFTER_FREEZE,
        'pose_hold_kp': GAConfig.POSE_HOLD_KP,
        'pose_hold_kd': GAConfig.POSE_HOLD_KD,
        'base_lower_distance': GAConfig.BASE_LOWER_DISTANCE,
        'base_lower_duration': GAConfig.BASE_LOWER_DURATION,
        'base_lower_start_delay': GAConfig.BASE_LOWER_START_DELAY,
        'contact_solref': list(GAConfig.CONTACT_SOLREF),
        'contact_solimp': list(GAConfig.CONTACT_SOLIMP),
        'target_angular_dof_damping': GAConfig.TARGET_ANGULAR_DOF_DAMPING,
        'target_linear_dof_damping': GAConfig.TARGET_LINEAR_DOF_DAMPING,
        'platform_slide_joint': GAConfig.PLATFORM_SLIDE_JOINT,
        'contact_weight': GAConfig.CONTACT_WEIGHT,
        'height_weight': GAConfig.HEIGHT_WEIGHT,
        'no_drop_weight': GAConfig.NO_DROP_WEIGHT,
    ...
    with open(readme_path, 'w') as f:
        ...
        f.write(f"- No-Drop: {GAConfig.NO_DROP_WEIGHT}\n")
