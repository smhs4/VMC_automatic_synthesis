...
def decode_genome(individual: creator.Individual) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for index in range(individual.shape[0]):
        if index < GAConfig.TENDON_NUM * 2:
            ...
            # Object sites now use flat indexing (not 3D grid)
            obj_idx = int(obj_i) % GAConfig.N_SITES_OBJECT

            # Arm sites still use 3D grid indexing
            ...
            if index < GAConfig.TENDON_NUM:
                # Left chopstick to object
                G.add_edge(f"left_chopstick_site_{arm_x}_{arm_y}_{arm_z}", f"ghost_target_site_{obj_idx}_0_0", 
                           type='spatial',
                           stiffness=stiffness,
                           damping=damping,
                           springlength=[0, 0.01])
            else:
                # Right chopstick to object
                G.add_edge(f"right_chopstick_site_{arm_x}_{arm_y}_{arm_z}", f"ghost_target_site_{obj_idx}_0_0", 
                           type='spatial',
                           stiffness=stiffness,
                           damping=damping,
                           springlength=[0, 0.01])
...
def build_model_from_genome(G, angle=None) -> MuJoCoSimulation:
    ...
    builder.add_mesh("tetrahedron_mesh", 
                     ...
                     scale=[0.002, 0.002, 0.002])  # STL files often need scaling

    builder.add_body("target", pos=[0.5, 0, 0.3],
                     quat=orientation,
                     geom_type="mesh", 
                     geom_mesh="tetrahedron_mesh",
    ...
    builder.add_body("ghost_target", pos=[0.5, 0, 0.3],
                     ...
                     geom_type="mesh",
                     geom_mesh="tetrahedron_mesh",
                     free_joint=True, geom_rgba=[1,1,1,0.1], intersection=False
                     )
    ...
    t0 = time.perf_counter()
    object_site_count = 0
    for i, pos in enumerate(object_sites):
        site_name = f"ghost_target_site_{i}_0_0"  # Keep naming convention for compatibility
        site_labels.append(site_name)
        builder.add_site(
            name=site_name,
...
def evaluate_fitness(individual: creator.Individual) -> Tuple[float,]:
    ...
    for trial in range(trial_num):  # Can do multiple trials if desired
        sim = build_model_from_genome(G, angle= (trial - (trial_num // 2)) * (math.pi / 6) )  # Vary starting angle slightly per trial
...
def run_evolution(visualize: bool = False, resume: str = None, plot_stats: bool = False, 
    ...
    log(f"  Sites per Object:     {GAConfig.N_SITES_OBJECT} ({GAConfig.N_SITES_OBJECT})")
    ...
    with open(readme_path, 'w') as f:
        ...
        f.write(f"- Sites per Object: {GAConfig.N_SITES_OBJECT} ({GAConfig.N_SITES_OBJECT})\n")
